...
func newRedisConfigForTest() *Config {
	...
	cfg.Redis.IdleTimeout = 1
	return cfg
}

func Test_newCacheManager_Redis_Success(t *testing.T) {
	cfg := newRedisConfigForTest()
...
func Test_Redis_PutGet_RoundTrip(t *testing.T) {
	cfg := newRedisConfigForTest()
	cm, _, err := cfg.newCacheManager(5)
	require.NoError(t, err)

	key := "redis-roundtrip-key"
	val := []byte("hello redis")
	require.NoError(t, cm.Set(context.Background(), key, val))

	// Redis는 네트워크 지연이 있을 수 있으므로 짧은 대기
...
func Test_Redis_TTL_Expires(t *testing.T) {
	cfg := newRedisConfigForTest()
...
func Test_Redis_Concurrency_NoRace(t *testing.T) {
	cfg := newRedisConfigForTest()
	cm, _, err := cfg.newCacheManager(5)
	require.NoError(t, err)

	keys := []string{"redis-k1", "redis-k2", "redis-k3", "redis-k4", "redis-k5"}
...
func Test_Redis_Marshal_Unmarshal_CacheValue(t *testing.T) {
	cfg := newRedisConfigForTest()
...
func Test_Redis_LoadOrStore_ReusesInstance(t *testing.T) {
	cfg := newRedisConfigForTest()
	...
	// Redis는 외부 서비스이므로 실제로는 같은 연결을 재사용
	key := "redis-reuse"
...
func Test_Redis_DifferentConfig_IsolatedInstances(t *testing.T) {
	cfg1 := newRedisConfigForTest()
	cfg2 := newRedisConfigForTest()
	cfg2.Redis.DBNumber = 1 // 다른 DB 사용
	...
	// 서로 다른 DB이므로 간섭하지 않아야 함
	require.NoError(t, cm1.Set(context.Background(), "redis-isolate-1", []byte("a")))
	require.NoError(t, cm2.Set(context.Background(), "redis-isolate-2", []byte("b")))

	time.Sleep(100 * time.Millisecond)

	v1Any, err := cm1.Get(context.Background(), "redis-isolate-1")
	require.NoError(t, err)
	v1, ok := v1Any.([]byte)
	require.True(t, ok)
	require.Equal(t, []byte("a"), v1)

	v2Any, err := cm2.Get(context.Background(), "redis-isolate-2")
	require.NoError(t, err)
	v2, ok := v2Any.([]byte)
	require.True(t, ok)
...
func Test_Redis_Connection_Error(t *testing.T) {
	cfg := newRedisConfigForTest()
	cfg.Redis.Host = "nonexistent-host"
	cfg.Redis.Port = 9999
	cfg.Redis.DialTimeout = 1 // 짧은 타임아웃으로 빠른 실패 유도

	cm, m, err := cfg.newCacheManager(5)
	// Redis 클라이언트는 지연 연결을 사용하므로 초기 생성 시에는 에러가 발생하지 않을 수 있음
